...
Привет! Это мой pet-проект, который я делал, чтобы пройти полный цикл разработки и развертывания приложения. В итоге получилась живая, работающая инфраструктура, которой я пользуюсь каждый день.
...
Этот проект стал для меня настоящим марафоном. Здесь и радость от первого запущенного кластера, и часы отладки сетевых проблем, и тот самый момент, когда наконец загорается зелёный замочек HTTPS.

Теперь у меня есть не просто код в репозитории, а живая, работающая система, которой я могу управлять из любой точки мира. Это и есть для меня DevOps — не просто набор слов, а реальный инструмент, который делает жизнь разработчика проще и интереснее.
