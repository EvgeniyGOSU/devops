...
5. **Написать Ansible-роли** для пост-настройки узлов (или использовать мои).
6. **Поднять Hysteria2 сервер** на VPS.
7. **Настроить WireGuard** и NGINX на VPS.
8. **Применить манифесты** из репозитория (`k8s/`, `cert-manager/`).
